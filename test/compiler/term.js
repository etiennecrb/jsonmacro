...
test('Number parsing', t => {
    t.deepEqual(rhs(t, compile('a = 8')), 8, 'it should parse one digit integers');
    t.deepEqual(rhs(t, compile('a = 82')), 82, 'it should parse any integer');
    t.deepEqual(rhs(t, compile('a = 8.2')), 8.2, 'it should parse floats');
    t.deepEqual(rhs(t, compile('a = 8.23')), 8.23, 'it should parse floats with multiple decimal places');
    t.deepEqual(rhs(t, compile('a = .2')), 0.2, 'it should parse floats without leading 0');
    t.deepEqual(
        rhs(t, compile('a = .23')),
        0.23,
        'it should parse floats without leading 9 but multiple decimal places'
    );
    t.deepEqual(rhs(t, compile('a = -2')), -2, 'it should parse negative integers');
    t.deepEqual(rhs(t, compile('a = -2.34')), -2.34, 'it should parse negative floats');
    t.deepEqual(rhs(t, compile('a = +25')), 25, 'it should parse "+" signed numbers');
    t.throws(
        () => rhs(t, compile('a = --2')),
        undefined,
    ...
    t.deepEqual(rhs(t, compile('a =  8  ')), 8, 'it should ignore whitespaces around numbers');
    t.deepEqual(rhs(t, compile('a=8')), 8, 'it should manage to parse numbers without whitespaces around');
    t.deepEqual(rhs(t, compile('a = (8)')), 8, 'it should parse numbers inside parentheses');
    t.end();
});

test('String parsing', t => {
    t.deepEqual(rhs(t, compile('a = "anything"')), 'anything', 'it should parse simple strings');
    t.deepEqual(
        rhs(t, compile('a = "this is a string"')),
        'this is a string',
        'it should parse strings with whitespaces'
    );
    t.deepEqual(rhs(t, compile('a = ""')), '', 'it should parse empty strings');
    t.throws(() => rhs(t, compile('a = \'anything\'')), undefined, 'it should throw error on single quoted strings');
    t.throws(
...
test('Array parsing', t => {
    t.deepEqual(rhs(t, compile('a = []')), [], 'it should parse empty arrays');
    t.deepEqual(rhs(t, compile('a = [8]')), [8], 'it should parse arrays with one element');
    t.deepEqual(rhs(t, compile('a = [8, 2]')), [8, 2], 'it should parse arrays with two elements');
    t.deepEqual(rhs(t, compile('a = [8,2,4,5]')), [8, 2, 4, 5], 'it should parse arrays with multiple elements');
    t.deepEqual(
        rhs(t, compile('a = [8, myVar, myFunc(), "string", (9), myVar.myMethod(), myVar.myProp, [1, 2]]')),
        [
            8,
            {[TYPE_VAR]: ['myVar']},
            {[TYPE_FUNC]: ['myFunc', []]},
    ...
    t.deepEqual(
        ...
        [{'+': [8, 1]}, {'or': [{'and': [{[TYPE_VAR]: ['a']}, {[TYPE_VAR]: ['b']}]}, {[TYPE_VAR]: ['c']}]}],
...
test('Function call parsing', t => {
    ...
    t.deepEqual(
        ...
        [{[TYPE_FUNC]: ['MyFunc', []]}, {[TYPE_FUNC]: ['$myFunc', []]}, {[TYPE_FUNC]: ['_myFunc', []]}],
        'it should allow any letter or "$" or "_" as first letter of function name'
    );
    t.throws(() => compile('1myFunc()'), undefined, 'it should not allow function name starting with a number');
    t.deepEqual(
        compile('myFunc(1, 5)'),
        [{[TYPE_FUNC]: ['myFunc', [1, 5]]}],
...
test('Variable parsing', t => {
    t.deepEqual(rhs(t, compile('a = myVar')), { [TYPE_VAR]: ['myVar'] }, 'it should parse simple variable');
    t.deepEqual(rhs(t, compile('a = b')), { [TYPE_VAR]: ['b'] }, 'it should allow variable name with one letter');
    t.deepEqual(
        rhs(t, compile('a = (b)')),
        { [TYPE_VAR]: ['b'] },
    ...
    t.throws(() => compile('a = 1myVar'), undefined, 'it should not allow variable name starting with a number');
    t.end();
});
...
test('Variable property and methods parsing', t => {
    ...
    t.throws(() => compile('a = b.1myProp'), undefined, 'it should not allow property name starting with a number');
    t.deepEqual(
    ...
    t.deepEqual(
        ...
        {[TYPE_CALL]: [{[TYPE_CALL]: [{[TYPE_VAR]: ['myVar']}, 'myMethod', []]}, 'myDeepMethod', []]},
    ...
    t.throws(() => compile('a = b.1myMethod()'), undefined, 'it should not allow method name starting with a number');
    t.deepEqual(
        rhs(t, compile('a = b.myMethod(1, 5)')),
        {[TYPE_CALL]: [{ [TYPE_VAR]: ['b'] }, 'myMethod', [1, 5]]},
        'it should parse method call with arguments'
    );
    t.deepEqual(
        rhs(t, compile('a = b.myMethod(8, myVar, myFunc(), "string", (9), myVar.myMethod(), myVar.myProp, [1, 2])')),
        {[TYPE_CALL]: [{ [TYPE_VAR]: ['b']},
            'myMethod', [
            8,
            {[TYPE_VAR]: ['myVar']},
            {[TYPE_FUNC]: ['myFunc', []]},
